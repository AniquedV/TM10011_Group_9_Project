...
best_svm_models = []
best_svm_estimators = {}  # Toegevoegde dictionary om beste modellen op te slaan
...
for name, model, param_grid in models_and_grids:
    ...
    best_svm_estimators[name] = grid.best_estimator_  # toegevoegde lijn voor beste model


#gecorrigeerd, vanaf hier startte origineel met oude paramters, gebruiken nu allemaal de estimators uit de grid search


print("\nLearning curves:")
fig, ax = plt.subplots(figsize=(8, 6))
plot_learning_curve(best_svm_estimators["linear"], "Learning Curve: SVM Linear", X_train, y_train, ax, cv=10)
plt.show()

fig, ax = plt.subplots(figsize=(8, 6))
plot_learning_curve(best_svm_estimators["poly"], "Learning Curve: SVM Poly", X_train, y_train, ax, cv=10)
plt.show()

fig, ax = plt.subplots(figsize=(8, 6))
plot_learning_curve(best_svm_estimators["rbf"], "Learning Curve: SVM RBF", X_train, y_train, ax, cv=10)
plt.show()

results_summary = []

for name, clf in best_svm_estimators.items():
